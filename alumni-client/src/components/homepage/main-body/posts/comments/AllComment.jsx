...
function AllComment({comments}) {
  ...
  return (
    <div className="w-full bg-stone-100 dark:bg-gray-700 p-4 h-50 overflow-auto mt-2 rounded-xl flex flex-col gap-4">
      {completeComments?.length>0 && completeComments.map((comment) => (
        <div key={comment._id} className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <div className="text-md text-gray-400">{comment?.content || "none"}</div>
          </div>
        </div>
      ))}
    </div>
